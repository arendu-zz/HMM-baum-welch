...
def get_viterbi_sequence(words):
    ...
    for k in range(1, len(words)):  # the words are numbered from 1 to n, 0 is special start character
        #print 'u beam is', u_beam, ' at k=', k
        #print 'full beam is', get_possible_states(words[k - 1]), ' at k=', k
        max_prob_to_state = {}
        for v in get_possible_states(words[k]): #[1]:
            ...
            max_p_for_v = max(max_prob_to_bt)
            max_bt = max_prob_to_bt[max_p_for_v]
            new_pi_key = (k, v)
            pi[new_pi_key] = max_p_for_v
            alpha_pi[new_pi_key] = logadd_of_list(sum_prob_to_bt)
            arg_pi[new_pi_key] = max_bt
            max_prob_to_state[max_p_for_v] = v
        u_beam = []
        top_half = sorted(max_prob_to_state)[-BEAM_WIDTH:]
        for th in top_half:
            u_beam.append(max_prob_to_state[th])
        #print 'pruned ', len(max_prob_to_state), '-->', len(u_beam), 'states at k=', k
