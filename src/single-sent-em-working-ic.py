...
def make_new_mle_estimates(posterior_bigrams_accumilation, posterior_obs_accumilation):
    #print 'updating MLEs...'
    global NEW_MLEs
    NEW_MLEs['N+1'] = CURRENT_MLEs['N+1']
    NEW_MLEs[OBS_VOCAB] = set(CURRENT_MLEs[OBS_VOCAB])  # makeing copy of vocab from current state #TODO how to update vocab with RAW?
    NEW_MLEs[STATE_VOCAB] = set(CURRENT_MLEs[STATE_VOCAB])  # TODO do I really need this line? state_vocab can't change because of RAW!

    #for k in posterior_obs_accumilation:  # Proper way to add to new , NEW = ORIGINAL + RAW-FB
    #    NEW_MLEs[k] = ORIGINAL_MLEs[k] + exp(posterior_obs_accumilation[k])
    #
    #for k in posterior_bigrams_accumilation:
    #    NEW_MLEs[k] = ORIGINAL_MLEs[k] + exp(posterior_bigrams_accumilation[k])

    for k in posterior_obs_accumilation:  # TEST way to add to new NEW = RAW-FB
...
if __name__ == "__main__":
    ...
    for i in range(11):
        ...
        tav, kav, uav = get_metrics(viterbi_predicted_tags, answer_tags, known)
        viterbi_perplexity_per_word = "%.3f" % (perpexity)
        '''
        display
        '''
        stderr.write(str('Tagging accuracy (Viterbi decoding): ' + str(tav) + '%\t'))
        stderr.write(str('(known: ' + str("%.2f" % kav) + '%\t'))
        stderr.write(str('novel: ' + str("%.2f" % uav) + '%)\n'))
        ...
        make_new_mle_estimates(posterior_bigrams_accumilation, posterior_obs_accumilation) # we make new mles with counts from f-b
