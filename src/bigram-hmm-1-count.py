...
def make_mle_estimates(filepath):
    CURRENT_MLEs['observations_vocab'] = set(['**OOV**'])  # will eventually store V these keys are also tuples
    ...
    for tagged_sentence in tagged_sentences:
        ...
        if tagged_sentence.strip() != '':
            for state_obs in tagged_sentence.strip().split(linesep):
                n += 1  # todo: this n does not count end-state or start-state n+s = tokens plus end_state counts
                obs = state_obs.split('/')[0]
                state = state_obs.split('/')[1]
                CURRENT_MLEs['observations_vocab'].add(obs)  # has a set of all seen observations [TYPES]
                CURRENT_MLEs[('observation_token_counts', obs)] += 1 #how many times have we seen obs
                CURRENT_MLEs['states_vocab'].add(state)  # has a set of all seen states
            ...
            s += 1  # n+s will include tokens plus end state i.e. ###

    CURRENT_MLEs[('backoff_transition', END_STATE)] = s / float(n+s)
    CURRENT_MLEs[('backoff_transition', START_STATE)] = s / float(n+s)
    ...
    for word in CURRENT_MLEs['observations_vocab']:
        if word == '**OOV**':
            pass
        backoff_for_observation = (CURRENT_MLEs[('observation_token_counts', word)] + 1) / float(
            n + s+ len(CURRENT_MLEs['observations_vocab']))
...
def one_count_probabilty(type, *args):
    if type == 'transition':
        ...
        if ('ocp_emission', obs, v) not in CURRENT_MLEs:
            ...
            if obs not in CURRENT_MLEs['observations_vocab']:
    ...
    for k in range(1, max(words) + 1):  # the words are numbered from 1 to n, 0 is special start character
        for v in possible_states[words[k]]:
            ...
            for u in possible_states[words[k - 1]]:
                ...
                print (k - 1, u), v, u, p, '=', q, '*', e, '*', pi[(k - 1, u)]
...
if __name__ == "__main__":
    ...
    #print 'known:', correct_known_tags, '/', total_known_tags, '=', known_accuracy
    #print 'novel:', unknown_accuracy
